...
for i=1:length(flacs)
    disp(flacs{i});
    [status, result] = unix(['curl -u ' username ':' password ' -X POST --header "Content-Type: audio/flac" --header "Transfer-Encoding: chunked" --data-binary @' flacs{i} ' "https://stream.watsonplatform.net/speech-to-text/api/v1/recognize?continuous=true"']);
